            ...
            <div className="mt-3 space-y-4">
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-2">
                <div className="rounded-xl border border-slate-200 bg-slate-50/60 p-3">
                  ...
                  <div className="flex items-center gap-2 mt-1 text-xl font-bold text-slate-900">
                    <BarChart3 size={16} className="text-indigo-500" />
                    {leadMetrics.total}
                  </div>
                  <p className="text-[11px] text-slate-500 mt-1">Across {activeSheet?.statuses.length || 0} statuses</p>
                </div>
                <div className="rounded-xl border border-amber-200 bg-amber-50 p-3">
                  <div className="text-[11px] uppercase text-amber-600 font-semibold">Waiting</div>
                  <div className="flex items-center gap-2 mt-1 text-xl font-bold text-amber-700">
                    <Clock size={16} />
                    {leadMetrics.waiting}
                  </div>
                  <p className="text-[11px] text-amber-700/80">Keep these moving</p>
                </div>
                <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-3">
                  <div className="text-[11px] uppercase text-emerald-700 font-semibold">Confirmed</div>
                  <div className="flex items-center gap-2 mt-1 text-xl font-bold text-emerald-700">
                    <Check size={16} />
                    {leadMetrics.confirmed}
                  </div>
                  <p className="text-[11px] text-emerald-700/80">Ready for onboarding</p>
                </div>
                <div className="rounded-xl border border-rose-200 bg-rose-50 p-3">
                  <div className="text-[11px] uppercase text-rose-600 font-semibold">Stale</div>
                  <div className="flex items-center gap-2 mt-1 text-xl font-bold text-rose-700">
                    <Activity size={16} />
                    {leadMetrics.stale}
                  </div>
                  <p className="text-[11px] text-rose-700/80">No update in 7+ days</p>
                </div>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div className="border border-slate-200 rounded-xl p-3 bg-slate-50/60">
                  <div className="flex items-center gap-2 text-sm text-slate-600 font-semibold mb-2">
                  ...
                  <div className="mt-3 flex justify-end">
                    <button
                      onClick={createSheet}
                      className="inline-flex items-center gap-2 px-3 py-2 bg-indigo-600 text-white rounded-lg shadow hover:bg-indigo-700 transition disabled:opacity-50"
                      disabled={!sheetForm.name.trim()}
                    >
                      <Plus size={14} /> Create sheet
                    </button>
                  </div>
                </div>
            ...
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              ...
              <div className="flex gap-2 w-full md:w-auto flex-wrap justify-end">
                ...
                <button
                  ...
                  className="px-3 py-2 rounded-lg border border-slate-200 text-[12px] text-slate-500 bg-white hover:border-slate-300"
                >
                  Statuses
                </button>
      ...
      {isStatusModalOpen && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-md p-5 space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 text-slate-800 font-semibold">
                <NotebookPen size={18} /> Manage statuses
              </div>
              <button
