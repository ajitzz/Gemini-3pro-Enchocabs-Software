...
app.post('/api/daily-entries', async (c) => {
  ...
  const q = `
    ...
    RETURNING *;
  `;
  const result = await query(c.env, q, [entryId || crypto.randomUUID(), isoDate, e.day, e.vehicle, e.driver, e.shift, e.qrCode, e.rent, e.collection, e.fuel, e.due, e.payout, e.notes]);
